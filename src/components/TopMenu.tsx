...
import KinesisLogo from "../assets/icons/logo.png"
...
type ActiveTabType = "" | "pools" | "risk" | "bridges" | "vesdl"
...
const NavMenu = styled(NavLink)<NavLinkProps & { selected: boolean }>(
  ({ theme, selected }) => {
    return {
      ...
      borderBottom: selected ? "3px solid " + theme.palette.info.main : "",
    }
  },
)
      ...
      >
        <Box display="flex" width="100%" alignItems="center">
          <Hidden lgDown>
            <Box flex={{ xl: 1 }}>
              <NavLink to="/">
                <img src={KinesisLogo} alt="logo" width={60} height={60} />
              </NavLink>
            </Box>
          </Hidden>
          <Stack
          ...
          >
            ...
            {/* <NavMenu to="/bridges" selected={activeTab === "bridges"}>
              {t("bridges")}
            </NavMenu> */}
            {/*{IS_DEVELOPMENT && (*/}
...
function RewardsButton({
  ...
  return IS_SDL_LIVE ? (
    <Button
      ...
      endIcon={<img src={"./logo.png"} alt="Logo" width={20} height={20} />}
    >
      {formattedTotal}
    </Button>
  ) : null
