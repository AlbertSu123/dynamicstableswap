import { CELER, POOLS_MAP, PoolName, TRANSACTION_TYPES } from "../constants"
  ...
  const approveAndStake = useCallback(
    async (amount: BigNumber) => {
      ...
      try {
        ...
        console.log(poolPid, amount, account)
    ...
    async function fetchAmount() {
      ...
      try {
        ...
        tokenAddresses.forEach((address, i) => {
          ...
          if (
            address.toLowerCase() === CELER.addresses[chainId].toLowerCase()
          ) {
            rewards[CELER.symbol] = tokenAmounts[i]
          }
        })
        setAmountOfSpaClaimable(rewards?.CELER || Zero)
      } catch (err) {
        console.error(err)
      }
    }
